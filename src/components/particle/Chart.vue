    ...
    return {
      data: {
        ...
        datasets: [
          {
            label: '潮の高さ',
            data: getTides(this.json, this.year + '-' + this.month + '-' + this.day),
            borderColor: '#ccf',
            backgroundColor: '#eef',
            type: 'line',
            lineTension: 0.1,
          ...
          {
            label: '潮平均値',
            data: getTideAverage(this.json, this.year + '-' + this.month + '-' + this.day),
            borderColor: '#ffcccc',
            fill: false,
            type: 'line',
          },
          {
            label: '現在時刻',
            data: getNow(getTides(this.json, this.year + '-' + this.month + '-' + this.day)),
            backgroundColor: '#336',
        ...
        scales: {
          ...
          yAxes: [{
            ...
            ticks: {
              beginAtZero: true,
              stepSize: 10,
              max: Math.ceil(Math.max.apply(null, getTides(this.json, this.year + '-' + this.month + '-' + this.day)) / 10 ) * 10 + 30,
            }
          }]
        }
      }
